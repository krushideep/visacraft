  ...
  return <Box sx={{ minHeight: '100vh', background: 'linear-gradient(180deg,#f7f8f5 0%,#fff 72%)' }}>
    <Header /><Container maxWidth="lg" sx={{ py: { xs: 3, md: 6 } }}><Stack spacing={3}>
      ...
      <Card sx={{ borderRadius: 3, border: '1px solid', borderColor: 'divider', boxShadow: 'none' }}><CardContent sx={{ p: { xs: 2, md: 3 } }}><Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
        <FormControl fullWidth><InputLabel>Passport</InputLabel><Select value={passport} label="Passport" onChange={e => { setPassport(e.target.value); setDestination(''); }}>{COUNTRIES.map(c => <MenuItem key={c.code} value={c.name}>{c.name}</MenuItem>)}</Select></FormControl>
        <FormControl fullWidth><InputLabel>Destination</InputLabel><Select value={destination} label="Destination" onChange={e => setDestination(e.target.value)}><MenuItem value=""><em>Click the map or choose…</em></MenuItem>{COUNTRIES.filter(c => c.name !== passport).map(c => <MenuItem key={c.code} value={c.name}>{c.name}</MenuItem>)}</Select></FormControl>
